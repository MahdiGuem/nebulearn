...
export function TrackLessonsClient({ trackId }: { trackId: string }) {
  ...
  const handleAIUpload = async () => {
    ...
    setAiUploadStatus("Uploading...");
    setAiError(null);

    try {
      const formData = new FormData();
      formData.append("pdf", aiFile);
      formData.append("title", aiFile.name);
      ...
      setAiDocumentId(data.documentId);
      setAiUploadStatus("Processing PDF...");

      pollDocumentProgress(data.documentId);
    } catch {
      setAiError("Failed to upload. Please try again.");
      setAiUploading(false);
    }
  };
  ...
  const pollDocumentProgress = async (documentId: string) => {
    const poll = async () => {
      try {
        const res = await fetch(`/api/documents/progress?documentId=${documentId}`);
        const data = await res.json();

        if (data.document) {
          setAiUploadProgress(data.document.progress);
          setAiUploadStatus(
            data.document.status === "ready"
              ? "Ready!"
              : data.document.status === "error"
              ? `Error: ${data.document.errorMessage}`
              : `Processing: ${data.document.progress}%`
          );

          if (data.document.status === "ready") {
            setAiUploading(false);
            generateQuizFromDocument(documentId);
            return;
          }

          if (data.document.status === "error") {
            setAiError(data.document.errorMessage || "PDF processing failed");
            setAiUploading(false);
            return;
          }
        }
        ...
        setAiError("Failed to check document progress");
        setAiUploading(false);
      }
    };
  ...
  const generateQuizFromDocument = async (documentId: string) => {
    ...
    try {
      ...
      setAiSelected(new Set(quiz.questions.map((_: unknown, i: number) => i)));
    } catch {
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="text-sm file:mr-3 file:px-3 file:py-1.5 file:rounded-md file:border file:border-purple-300 file:bg-purple-100 file:text-purple-700 file:text-sm file:font-medium hover:file:bg-purple-200 file:cursor-pointer disabled:opacity-50"
                />
                <button
                  onClick={handleAIUpload}
                  disabled={!aiFile || aiUploading}
                  className="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded text-sm disabled:opacity-50"
                >
                  {aiUploading ? "Uploading..." : "Upload PDF"}
                </button>
              </div>
              {(aiUploading || aiUploadProgress > 0) && (
                <div className="space-y-2">
                  <div className="h-2 rounded-full bg-purple-200">
                    <div
                      className="h-2 rounded-full bg-purple-600 transition-all duration-300"
                      style={{ width: `${aiUploadProgress}%` }}
                    />
                  </div>
                  <p className="text-sm text-purple-600">
                    {aiUploadStatus} ({aiUploadProgress}%)
                  </p>
                </div>
              )}
              {aiGenerating && (
              ...
              {formOptions.map((opt, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  {formOptions.length > 2 && (
                    <button
                      type="button"
                      className="text-xs text-red-500 h-8 px-2 hover:bg-red-50 rounded"
                      onClick={() => {
            ...
            <div key={lesson.id} className="rounded-lg border overflow-hidden">
              ...
              {expandedLessonId === lesson.id && (
                <div className="border-t px-4 py-4 bg-muted/30 space-y-3">
                  ...
                  {(lesson.targetAttributes?.length ?? 0) > 0 && (
                    <p className="text-xs text-muted-foreground">
